# 4.2.1: Temporal Server Metrics Dashboard
...
workflows are hanging for an unknown reason. Our first action is to open this dashboard and begin looking through the available metrics. We notice spikes in the shard distribution and we know shard distribution is relevant to the history service. We confirm that the history service is having issues in the additional panels within this dashboard, such as Overview and Persistence, and then move on to dig deeper by opening the History Dashboard which contains more information.
