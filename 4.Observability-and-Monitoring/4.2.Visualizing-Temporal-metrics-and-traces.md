...
**Client Latency ($Service -> $Client)** tracks the 95th percentile latency of client-initiated operations to a Temporal service, highlighting high-latency interactions for specific client-service pairs and operations.

_Interpreting the Data_

1. The panel shows the 95th percentile (p95) latency for various client operations using:
    - `histogram_quantile(0.95, sum(rate(client_latency_bucket[5m])) by (operation, le))`
        - Uses Prometheus histogram quantiles to focus on higher-latency client interactions, aggregating over 5 minutes by both operation and latency bucket (`le`).
2. Operations tracked include:
    - `HistoryClientStartWorkflowExecution`: Latency to start workflow executions.
    - `MatchingClientCancelOutstandingPoll`: Latency of canceling outstanding polls.
    - `MatchingClientListNexusEndpoints`: Time to list Nexus endpoints.
    - `MatchingClientPollActivityTaskQueue`: Latency polling activity task queue.
    - `MatchingClientPollWorkflowTaskQueue`: Latency polling workflow task queue.
3. The use of `$Service` and `$Client` variables allows for targeted analysis of specific Temporal service and client interactions.
4. Insights and patterns to note:
    - Elevated or spiking latency for certain operations or periods may indicate backend slowdowns, network issues, or client processing bottlenecks.
    - Compare latency profiles across different operations to identify performance outliers.

This panel is useful for getting the health of the Temporal service at a glance and can be used to begin isolating problems affecting Temporal services.

_Correlation and Usefulness_
This panel is crucial for monitoring client experience and performance when interacting with Temporal services. It helps operators, SREs, and developers pinpoint latency issues per operation, prioritize optimizations, and detect performance regressions or pressure points. Correlating latency spikes with error panels or request-volume trends enables rapid diagnosis of systemic or client-specific slowdowns.